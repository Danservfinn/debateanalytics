      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            {/* Sticky verification indicator */}
            {claim.sticky && (
              <Badge variant="neutral" className="text-xs bg-primary/20 text-primary border-primary/30">
                <Shield className="w-3 h-3 mr-1" />
                {claim.fromCache ? "Cached" : "Permanent"}
              </Badge>
            )}
                ...
                {claim.verifiedAt && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span>
                      ...
                      {claim.verifiedBy && ` by ${claim.verifiedBy}`}
                    </span>
                    {claim.fromCache && (
                      <span className="text-primary">(from global registry)</span>
                    )}
                  </div>
                )}
                {claim.registryId && (
...
export function ClaimMatrix({ claims, className }: ClaimMatrixProps) {
  ...
  const stats = {
    ...
    sticky: claims.filter(c => c.sticky).length,
    fromCache: claims.filter(c => c.fromCache).length
  }
    ...
    >
      <Card variant="premium">
        ...
        <CardContent className="space-y-6">
          ...
          {/* Sticky Verifications Indicator */}
          {stats.sticky > 0 && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
              <Shield className="w-5 h-5 text-primary" />
              <div className="flex-1">
                <p className="text-sm font-medium text-primary">
                  {stats.sticky} permanent verification{stats.sticky !== 1 ? 's' : ''}
                </p>
                <p className="text-xs text-muted-foreground">
                  {stats.fromCache > 0
                    ? `${stats.fromCache} from global registry (shared across all analyses)`
                    : "These verifications are now part of the global registry"}
                </p>
              </div>
            </div>
          )}
