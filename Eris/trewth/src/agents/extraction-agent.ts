 ...
 *
 * Uses Mercury Parser API as fallback for complex HTML
 */
...
2. Content structure: headline (string), subhead (string or null), lede (string), body (string or array), sections (array)
...
No markdown code blocks, no explanations, just the raw JSON.`
  ...
  const userPrompt = `Extract structured data from this article HTML:\n\n${articleHtml.substring(0, 100000)}`
