...
7. "What AI Thinks" - your candid, neutral assessment of the article's credibility`
  ...
  // Validate scores
  const evidenceQuality = Math.max(0, Math.min(40, data.evidenceQuality || 0))
  const methodologyRigor = Math.max(0, Math.min(25, data.methodologyRigor || 0))
  const logicalStructure = Math.max(0, Math.min(20, data.logicalStructure || 0))
  const manipulationAbsence = Math.max(0, Math.min(15, data.manipulationAbsence || 0))
  ...
  return {
    ...
    whatAiThinks: data.whatAiThinks || 'Analysis incomplete',
    credibility: data.credibility || 'moderate',
  }
}
